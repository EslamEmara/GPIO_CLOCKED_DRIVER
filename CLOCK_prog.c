...
void CLOCK_voidInit()
{
        ...
        SETBIT(RCC , 6);        //(SETTING XTAL TO 16MHz)
        SETBIT(RCC,8);
        SETBIT(RCC,10);
        CLRBIT(RCC,7);
        CLRBIT(RCC,9);
...
#if (PLL_MODE == CLOCK_ENABLE)
        CLRBIT(RCC , 11);               //BYPASS
        CLRBIT(RCC , 13);
#elif (PLL_MODE == CLOCK_DISABLE)
        SETBIT(RCC , 11);
...
#if (SYS_DIV == SYSDIV_1 && PLL_MODE == CLOCK_DISABLE)
        CLRBIT(RCC,23);
        CLRBIT(RCC,24);
        CLRBIT(RCC,25);
        CLRBIT(RCC,26);
#elif (SYS_DIV == SYSDIV_2 && PLL_MODE == CLOCK_DISABLE)
        SETBIT(RCC,23);
        CLRBIT(RCC,24);
        CLRBIT(RCC,25);
        CLRBIT(RCC,26);
